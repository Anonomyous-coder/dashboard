...
(function () {
  ...
  function accountDBMount(root) {
    ...
    root.querySelector("#profileFormDB").onsubmit = async (e) => {
      e.preventDefault();
      const f = e.target;
      const patch = { full_name: f.full_name.value.trim(), title: f.title.value.trim(), phone: f.phone.value.trim(), location: f.location.value.trim(), dept: f.dept.value.trim() };
      ...
      <div class="grid cols-3 mt-24">
        ${list.length ? list.map((m) => `
          <div class="card card-pad">
            <div class="flex between center">
              <div class="flex gap-12 center">
                <div class="avatar" style="width:46px;height:46px;font-size:16px">${U.initials(window.DB.displayName(m))}</div>
